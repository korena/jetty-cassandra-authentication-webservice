...
public class UsersDaoImpl implements UsersDao, Serializable {
    ...
    UsersDaoImpl(){
        ...
        // cdi bean manager cant scan datastax classes for some reason ...
...
//        fields should be checked for null values ... but you get the idea ...
    ...
    public Users getUserById(UUID uid) {
        ...
        System.out.println(String.format("%-30s\t%-20s\t%-20s\t%-20s\t%-20s\n%s", "uid", "ualias", "fname", "lname",
                "address",
                "-------------------------------+-----------------------+--------------------"));
        Row row = queryResultFinal.one();
        System.out.println(String.format("%-30s\t%-20s\t%-20s\t%-20s\t%-20s", row.getUUID("uid"),
                row.getString("ualias"), row.getString("fname"), row.getString("lname"), row.getString("address")));
